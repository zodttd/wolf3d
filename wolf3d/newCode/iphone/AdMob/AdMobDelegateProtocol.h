...
// When a user clicks on an ad for a web page, should it be loaded in an
// AdMob-provided embedded browser? Defaults to YES (use AdMob embedded browser).
// If this is implemented and returns NO, then Safari will be launched, unless
// you implement presentWebView: (see below).
// This method must _always_ return the same value; varying the value may lead
// to unexpected results.
- (BOOL)useEmbeddedWebView;

// If you already have a mini-browser built into your application, and
// you would like to use that to open web links rather than AdMob's mini-browser
// or Safari, implement this method. Does not get called if useEmbeddedWebView returns YES.
// Remember that ads can be directed to many different urls, so if your mini-browser
// has logic specific to your site, you should be prepared to handle web requests
// across a variety of websites, as well as to launch iTunes, the AppStore, YouTube,
// etc. as appropriate.
- (void)presentWebView:(NSURL *)adLandingPage;

// If you're using the AdMob mini-browser, you can control the appearance of
// the nav bars, to fit with the rest of your app. See the documentation for UIToolbar
...
// Sent just before presenting a canvas page or an embedded webview. Use this
// opportunity to (say) stop animations, time sensitive interactions, etc.
- (void)willPresentModalCanvas;
- (void)willPresentWebView;

// Sent just after dismissing a canvas page or an embedded webview. Use this opportunity to
